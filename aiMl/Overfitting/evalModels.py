...
<center>
    <img src = aiMl/Overfitting/images/pipes.png width = 50% />
</center>
'''
import pandas as pd
import numpy as np
...
train = pd.read_csv('aiMl/Overfitting/data/train.csv')
test = pd.read_csv('aiMl/Overfitting/data/test.csv')
...
highest_corr = correlations.index[1]  # The first index is 'SalePrice' itself
...
print(X_train[['OverallQual', 'CentralAir', 'HeatingQC']].head())
...
# Answer check
print(pipe_2_train_mse)
print(pipe_2_test_mse)
...
for i in range(1, 6):
    ...
    train_preds = pipe.predict(X_train[features])
    test_preds = pipe.predict(X_test[features])
...
best_complexity = test_mses.index(min(test_mses)) + 1  # +1 because degrees start from 1
